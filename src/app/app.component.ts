...
export class AppComponent implements OnInit {
  ...
  verify() {
    ...
    this.persons.forEach(person => {
      if (!this.verifyHandluggage(person)) {
        verified = false;
      }
      if (!this.verifyTotalNoneSpecialItems(person)) {
        verified = false;
      }
      person.items.forEach(item => {
        if (!this.verifyItemsTotalWeight(item, person)) {
          verified = false;
        }
      });
    });
    if (verified) {
      alert("Ser bra ut. Nice work hero!");
    }
  }
  ...
  private sumTotalWeightPersonsItems(persons: IPerson[]) {
    let totalSum = persons.reduce(function(acc, cur) {
